...
const createCustomerAddressesTableQuery = (
  ...
  const customerAddresses = config.tables?.customerAddresses?.name as string;
  const customers = config.tables?.customers?.name as string;

  return sql.unsafe`
    CREATE TABLE IF NOT EXISTS ${sql.identifier([customerAddresses])} (
...
const createCustomerUsersTableQuery = (
  ...
  const customerUsers = config.tables?.customerUsers?.name as string;

  return sql.unsafe`
    CREATE TABLE IF NOT EXISTS ${sql.identifier([customerUsers])} (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      customer_id VARCHAR(36) NOT NULL,
...
const createCustomerInvitationsTableQuery = (
  config: SaasConfig,
): QuerySqlToken<ZodTypeAny> => {
  const customerInvitations = config.tables?.customerInvitations
    ?.name as string;

  return sql.unsafe`
    CREATE TABLE IF NOT EXISTS ${sql.identifier([customerInvitations])} (
